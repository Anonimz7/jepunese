...
filterBtn.addEventListener('click', () => {
  ...
  categoryFilter.disabled = !categoryFilter.disabled;
});

// Menambahkan logika Filter
categoryFilter.addEventListener('change', () => {
  const selectedCategory = categoryFilter.value;

  if (selectedCategory) {
    filteredData = kanjiData.filter(item => item.kategori === selectedCategory);
  } else {
    filteredData = [...kanjiData]; // Reset jika filter kategori kosong
  }
...
// Memperbarui Fungsi HandleSearch untuk Menyesuaikan Filter
function handleSearch() {
  const keyword = searchInput.value.toLowerCase().trim();
  const selectedCategory = categoryFilter.value;

  filteredData = kanjiData.filter(item => {
    const matchesSearch = item.kanji.includes(keyword) ||
                          item.furigana.toLowerCase().includes(keyword) ||
                          item.romaji.toLowerCase().includes(keyword) ||
                          item.indo.toLowerCase().includes(keyword) ||
                          item.inggris.toLowerCase().includes(keyword);
